...
def generate_chunk(ichunk):
    ...
    data_path = f'{data_dir}/powerspectra_11param_{ichunk*chunksize:09d}_{(ichunk+1)*chunksize:09d}.h5'
    ...
    print(f"SAVING DATA {ichunk}. Time elapsed: {telapsed:.2f}, per sample={telapsed/chunksize}")
    ...
    with h5py.File(data_path, 'w') as f:
        f.create_dataset("k", data=k.astype(np.float32))    
        f.create_dataset("model_params", data=model_params[:, useful_param_inds].astype(np.float32))
        f.create_dataset("model_param_names", data=np.array(list(param_priors.keys()))[useful_param_inds].astype('S'))
        
        f.create_dataset("pk0_sig", data=P_err[0].astype(np.float32))
        f.create_dataset("pk2_sig", data=P_err[1].astype(np.float32))
        f.create_dataset("pk4_sig", data=P_err[2].astype(np.float32))

        f.create_dataset("pk0_lin", data=pk0_lin.astype(np.float32))
        f.create_dataset("pk2_lin", data=pk2_lin.astype(np.float32))
        f.create_dataset("pk4_lin", data=pk4_lin.astype(np.float32))
        
        f.create_dataset("pk0_nonlin", data=pk0_nonlin.astype(np.float32))
        f.create_dataset("pk2_nonlin", data=pk2_nonlin.astype(np.float32))
        f.create_dataset("pk4_nonlin", data=pk4_nonlin.astype(np.float32))

        f.create_dataset("pk0_nonlin_convolved", data=pk0_nonlin_convolved.astype(np.float32))
        f.create_dataset("pk2_nonlin_convolved", data=pk2_nonlin_convolved.astype(np.float32))
        f.create_dataset("pk4_nonlin_convolved", data=pk4_nonlin_convolved.astype(np.float32))

        f.create_dataset("pk0_r", data=pk0_r.astype(np.float32))
        f.create_dataset("pk2_r", data=pk2_r.astype(np.float32))
        f.create_dataset("pk4_r", data=pk4_r.astype(np.float32))

        f.create_dataset("pk0_r_sig", data=pk0_r_sig.astype(np.float32))
        f.create_dataset("pk2_r_sig", data=pk2_r_sig.astype(np.float32))
        f.create_dataset("pk4_r_sig", data=pk4_r_sig.astype(np.float32))
